...
def doRatioHists(pspec,pmap,total,totalSyst,maxRange,fixRange=False,fitRatio=None,errorsOnRef=True,ratioNums="signal",ratioDen="background",ylabel="Data/pred.",doWide=False,showStatTotLegend=False):
    numkeys = [ "data" ]
    print "RATIOS!!!"
    if True:#True: #False: #True: #"data" not in pmap: 
        print ratioDen, pmap
    ...
    unity.SetFillColor(ROOT.kOrange-4);
    unity.SetLineColor(ROOT.kOrange-4);
    unity.SetMarkerStyle(1);
    unity.SetMarkerColor(ROOT.kOrange-4);
    unity0.SetFillStyle(1001);
    unity0.SetFillColor(ROOT.kCyan);
    unity0.SetLineColor(ROOT.kCyan);
    unity0.SetMarkerStyle(1);
    unity0.SetMarkerColor(ROOT.kCyan);
    ...
    if binlabels != "" and len(binlabels.split(",")) == unity.GetNbinsX():
        ...
        unity.GetXaxis().SetLabelSize(pspec.getOption("xBinLabelSize",0.15))
        unity.GetXaxis().SetLabelOffset(0.012)
    ...
    if not options.ratiolegend0position: leg0 = ROOT.TLegend(0.12 if doWide else 0.2, 0.8, 0.25 if doWide else 0.45, 0.9)
    else: leg0 = ROOT.TLegend(options.ratiolegend0position[0], options.ratiolegend0position[1],options.ratiolegend0position[2],options.ratiolegend0position[3])
    leg0.SetFillColor(0)
    leg0.SetShadowColor(0)
    leg0.SetLineColor(0)
    leg0.SetTextFont(42)
    leg0.SetTextSize(0.035*0.7/0.3)
    leg0.AddEntry(unity0, "Stat. pred. unc.", "F") if not options.posfitlike else leg0.AddEntry(unity0, "total unc.", "F") 
    #if showStatTotLegend and not options.posfitlike: leg0.Draw()
    #leg0.Draw()
    if not options.ratiolegend1position: leg1 = ROOT.TLegend(0.25 if doWide else 0.45, 0.8, 0.38 if doWide else 0.7, 0.9)
    else: leg1 = ROOT.TLegend(options.ratiolegend1position[0], options.ratiolegend1position[1],options.ratiolegend1position[2],options.ratiolegend1position[3])
    leg1.SetFillColor(0)
    leg1.SetShadowColor(0)
    leg1.SetLineColor(0)
    leg1.SetTextFont(42)
    leg1.SetTextSize(0.035*0.7/0.3)
    leg1.AddEntry(unity, "Total pred. unc.", "F") if not options.posfitlike else 0
...
def doLegend(pmap,mca,corner="TR",textSize=0.035,cutoff=1e-2,cutoffSignals=True,mcStyle="F",legWidth=0.18,legBorder=True,signalPlotScale=None,totalError=None,header="",doWide=False,options=None):
        ...
        for p in backgrounds:
            ...
            if p in pmap and pmap[p].Integral() >= cutoff*total:
                ...
                if p in options.excludeFromLegend: continue
        ...
        if totalError: entries.append((totalError,"Total bkg. unc.","LF"))
                        ...
                        for proc in pmap.keys():
                            ...
                            if True:
                                ...
                                if options.posfitlike:  
                                    ...
                                    toAdd  = tf.Get(pspec.name+"_"+proc)
                                    for merge in options.plotmergemap:
                                      dest = merge.split("=")[0][:-1]
                                      orgs = merge.split("=")[1].split(",")
                                      if proc == dest:
                                        print "Merging %s into %s"%(",".join(orgs), dest)
                                        for orig in orgs:
                                          toAdd.Add(tf.Get(pspec.name+"_"+orig))

                                if type(toAdd) == type(falsehist): continue

                                print "Adding %s from file"%proc
...
#                        if plot.Integral() <= 0: continue
                ...
                if options.showMCError:
                    totalError = doShadedUncertainty(totalSyst) if not options.posfitlike else doShadedUncertainty(total)
                ...
                if self._options.doOfficialCMS:
                    CMS_lumi.lumi_13TeV = "%i fb^{-1}" % self._options.lumi
                ...
                if doRatio:
                  ...
                  if options.vertLines:
                    ...
                    ratexts = []
                    for item in lines.items: #List of dictionaries with the input
                        if not(item["doOnRatio"]): continue
                        if item["mode"] == "range":
                            print "Drawing...", item["name"], tot_bins, item["before"],item["after"]
                            if item["before"] != 1:
                               x = total.GetBinLowEdge(item["before"])
                               y1 = options.maxRatioRange[0]
                               y2 = options.maxRatioRange[1]
                               print "before", x,y1,x,y2
                               ratbef.append(ROOT.TLine(x,y1,x,y2))
                               ratbef[-1].SetLineStyle(item["style"])
                               ratbef[-1].SetLineWidth(item["widht"])
                               ratbef[-1].SetLineColor(item["color"])
                               ratbef[-1].Draw("same")
                            if item["after"] != tot_bins:
                               x = total.GetBinLowEdge(item["after"]+1)
                               y1 = options.maxRatioRange[0]
                               y2 = options.maxRatioRange[1]
                               print "after", x,y1,x,y2
                               rataft.append(ROOT.TLine(x,y1,x,y2))
                               rataft[-1].SetLineStyle(item["style"])
                               rataft[-1].SetLineWidth(item["widht"])
                               rataft[-1].SetLineColor(item["color"])
                               rataft[-1].Draw("same")
                        if item["mode"] == "text" and "forceRatio" in item.keys():
                            print "Plotting in ratio...", item["name"]
                            ratexts.append(ROOT.TLatex(item["x"], item["y"], item["text"]))
                            if "angle" in item.keys(): ratexts[-1].SetTextAngle(item["angle"])
                            ratexts[-1].SetTextSize(item["size"])
                            ratexts[-1].Draw("same")
...
def addPlotMakerOptions(parser, addAlsoMCAnalysis=True):
    ...
    parser.add_option("--ratiolegend0position", dest="ratiolegend0position", type="float", nargs=4, default=None, help="Legend position in the ratio for stat. uncertainty")
    parser.add_option("--ratiolegend1position", dest="ratiolegend1position", type="float", nargs=4, default=None, help="Legend position in the ratio for total uncertainty")
    parser.add_option("--excludeFromLegend", dest="excludeFromLegend", action="append", default=[], help="Processes not to be added to the legend")
