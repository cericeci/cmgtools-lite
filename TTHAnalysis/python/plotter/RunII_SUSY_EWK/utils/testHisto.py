...
pullsNorm      = ROOT.TH1F("pulls_norm", "pulls_norm", 50,-5,5)
pullsNaive     = ROOT.TH1F("pulls_naive", "pulls_naive", 50,-5,5)
pullsQuantiles = ROOT.TH1F("pulls_quant", "pulls_quant", 50,-5,5)
...
def produceVals(inF,dM):
  ...
  thePois = []
  num = []
  inC = inF.Get("data_obs_central_dM%i"%dM)
  for b in range(1,21):
...
def produceHistos(dM):
  ...
  for i in range(len(valuesNorm)):
    ...
    valuesNorm[i].Fit(thePoisNorm[i])
    ...
    c.SaveAs("~/www/plots_bootstrap/check_dM%i_bin%i.pdf"%(dM,i))
    c.SaveAs("~/www/plots_bootstrap/check_dM%i_bin%i.png"%(dM,i))
...
tl.Draw("same")
c.SaveAs("~/www/plots_bootstrap/pulls_all.pdf")
c.SaveAs("~/www/plots_bootstrap/pulls_all.png")
...
tl.Draw("same")
c.SaveAs("~/www/plots_bootstrap/pvals_all.pdf")
c.SaveAs("~/www/plots_bootstrap/pvals_all.png")
