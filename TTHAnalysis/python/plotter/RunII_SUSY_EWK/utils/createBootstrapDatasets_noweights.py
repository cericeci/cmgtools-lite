...
central = "/nfs/fanae/user/carlosec/CMSSW_10_4_0/src/CMGTools/TTHAnalysis/python/plotter/LEEPFix_toys_central_dM[M]/cuts_3l.input.root"
varsTemp =  "/nfs/fanae/user/carlosec/CMSSW_10_4_0/src/CMGTools/TTHAnalysis/python/plotter/[TAG]_dM[M]/cuts_3l.input.root"

tags = {"LEEPFix_toys_central":[0,0]} # {"LEEPFix_v2_toys_set1":[1,100]}#, "LEEPFix_toys_set2":[101,200], "LEEPFix_toys_set3":[201,300], "LEEPFix_toys_set4":[301,400], "LEEPFix_toys_set5":[401,500], "LEEPFix_toys_set6":[501,750], "LEEPFix_toys_set7":[751,999]}


signalInjections = ["x_TChiWZ_200_110","x_TChiWZ_350_1","x_TChiWZ_175_125"]

print signalInjections
outDatasets = "2016_dataobs_aspupi_bootstrap_noweights_signalInjected_w0.root"
...
def extract_histos(fil, m, ref=False, w=0, addHisto=False):
  ...
  for p in processes:
    ...
    if w == 0: hName = hName.replace("[W]","_weights_0Up")
    else: 
  ...
  if ref==False:
    ...
    data_obs.SetName("data_obs_weight0_dM"+m) #or _central_
    for p in centH.keys():
    ...
    if toAdd:
      ...
      extra = ""
    print "Extra: ", extra, addHisto, toAdd
...
for s in signalInjections:
  for m in masses:
    ...
    for t in tags:
      for i in range(tags[t][0], tags[t][1]+1):
        extract_histos(varsTemp.replace("[TAG]",t), str(m), True,i,["/nfs/fanae/user/carlosec/CMSSW_10_4_0/src/CMGTools/TTHAnalysis/python/plotter/LEEPFix_toys_signals_dM[M]/cuts_3l.input.root".replace("[M]",str(m)),s])
