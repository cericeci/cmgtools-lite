...
for page in xrange(n):
    ...
    for p in xrange(n_params):
        ...
        max_impact_3 = max(
            max_impact_3, abs(imp_3[1] - imp_3[0]), abs(imp_3[2] - imp_3[1]))
    ...
    plot.DrawCMSLogo(pads[0], 'CMS', args.cms_label, 0, 0.4, 0.00, 0.00)
    s_nom, s_hi, s_lo = GetRounded(POI_fit[1], POI_fit[2] - POI_fit[1], POI_fit[1] - POI_fit[0])
    s_nom1, s_hi1, s_lo1 = GetRounded(fLR_fit[1], fLR_fit[2] - fLR_fit[1], fLR_fit[1] - fLR_fit[0])
    s_nom2, s_hi2, s_lo2 = GetRounded(fO_fit[1], fO_fit[2] - fO_fit[1], fO_fit[1] - fO_fit[0])
    if not args.blind:
        plot.DrawTitle(pads[1], '#hat{#mu} = %s^{#plus%s}_{#minus%s}%s' % (
            s_nom, s_hi, s_lo,
            '' if args.units is None else ' '+args.units), 3, 0.27)
        plot.DrawTitle(pads[2], '#hat{f_{LR}} = %s^{#plus%s}_{#minus%s}%s' % (
            s_nom1, s_hi1, s_lo1,
            '' if args.units is None else ' '+args.units), 3, 0.27)
        plot.DrawTitle(pads[3], '#hat{f_{O}} = %s^{#plus%s}_{#minus%s}%s' % (
            s_nom2, s_hi2, s_lo2,
            '' if args.units is None else ' '+args.units), 3, 0.27)
