...
if [ "$ASIMOV" = "d" ]; then
   echo "Using data"
   ASIMOV=" --setParameters fLR=-0.116,fO=0.258,mu=1  "
   PREFIX="d"
fi
...
if [ "$ASIMOV" = "t1" ]; then
   ...
   ASIMOV=" -t -1 --setParameters fLR=-0.116,fO=0.258,mu=1  "
   PREFIX="t1"
fi
...
combineTool.py -M Impacts -d $HERE/${DATACARD} --doInitialFit --robustFit 1 $ASIMOV $EXTRA -m 1 -n $PREFIX --redefineSignalPOIs fLR,fO,mu --floatOtherPOIs 1 


echo "Second Stage: fit scan for each nuisance parameter"
echo "--------------------------------------------------"

combineTool.py -M Impacts -d $HERE/${DATACARD} --robustFit 1 --doFits --parallel $JOBS $ASIMOV $EXTRA -m 1 -n $PREFIX  --redefineSignalPOIs fLR,fO,mu

echo "Third Stage: collect outputs"
echo "----------------------------"

combineTool.py -M Impacts -d $HERE/${DATACARD} -o impacts$PREFIX.json $ASIMOV $EXTRA -m 1 -n $PREFIX --redefineSignalPOIs fLR,fO,mu

echo "Fourth Stage: plot pulls and impacts"
echo "------------------------------------"

python plotImpacts_multiSignal.py -i impacts$PREFIX.json -o impacts$PREFIX --left-margin 0.2 --POI mu
